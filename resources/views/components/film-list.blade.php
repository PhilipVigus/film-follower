...
<div 
    ...
    x-data="{ 
        films: {{ $films }},
        highlightedFilm: {{ $highlightedFilmId }},
        searchTerm: '',
        currentTerm: '',
        fuse: null,
        filmsPerSlice: 10,
...
>    
    ...
    <template x-for="(result, index) in currentTerm ? fuse.search(currentTerm).slice(0, filmsShowing) : films.slice(0, filmsShowing)" :key="index">
        <article class="mt-8 bg-gray-200 h-auto shadow-md overflow-hidden rounded-md p-6" :class="{'bg-yellow-100': highlightedFilm && index === 0}" :index="index">
