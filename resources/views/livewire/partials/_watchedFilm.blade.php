<article class="mt-8 bg-gray-200 h-auto shadow-md overflow-hidden rounded-md p-6" wire:key="{{ $loop->index }}">
    ...
    <div class="flex space-x-6 mt-4">
        <section class="w-1/2">
            ...
            <button class="w-full bg-gray-300 p-2 rounded-md hover:bg-gray-400" wire:click="$emitTo('modal', 'open', 'remove-review', { film: {{ $film }} })">Remove review</button>
                <button class="w-full bg-blue-800 text-gray-100 p-2 rounded-md hover:bg-blue-900" wire:click="$emitTo('modal', 'open', 'review-details', { film: {{ $film }} })">Edit review</button>
            </div>
        </section>
        
        <div class="w-1/2">
            <section class="bg-gray-400 rounded-md p-4 shadow-md">
                <h3 class="font-bold text-lg">Shortlist</h3>
            ...
            <section class="bg-gray-400 rounded-md p-4 shadow-md mt-2">
                <h3 class="font-bold text-lg">Review</h3>

                <div class="mt-2">
                    @for ($i=0; $i < $film->reviews->first()->rating; $i++)
                        <svg 
                            xmlns="http://www.w3.org/2000/svg" 
                            viewBox="0 0 20 20" 
                            class="w-6 h-6 inline-block fill-current text-yellow-500"
                ...
                <div class="mt-2">{{$film->reviews->first()->comment }}</div>
            </section>
