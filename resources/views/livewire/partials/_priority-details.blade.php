<section class="bg-gray-400 rounded-md p-4 shadow-md mb-4 relative">
    ...
    <template x-for="(item) in Array.from(Array(result.item.priorities[0].rating))">
        <svg 
    ...
    <div class="mt-2" x-text="result.item.priorities[0].comment"></div>

    <button x-on:click="$wire.emitTo('modal', 'open', 'priority-details', { film: result.item })">
